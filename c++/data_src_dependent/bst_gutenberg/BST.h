...
template <typename Key, typename E>
BSTElement<Key, E>* BST<Key, E>::inserthelp(BSTElement<Key, E>* root, const Key& k, 
									const E& it) {
	if (root == NULL) { // Empty tree: create node
		BSTElement<Key, E> *el = new BSTElement<Key, E>(k, it);
						// set a descriptive title
		GutenbergBook gb = el->getValue();
		vector<string> genre = gb.getGenre();
		vector<string> lang = gb.getLang();
		string label = gb.getTitle() + "\\n" + 
				"Num Words:" + to_string(gb.getNumWords()) +
				" Num Downloads:" + to_string(gb.getNumDownloads()) + "\\n"
				+ "Genres: ";
		for (int k = 0; k < genre.size(); k++) {
			label += genre[k];	
			label += (k == genre.size()-1) ? "" : ",";
		}
		label += "\\n Languages: ";
