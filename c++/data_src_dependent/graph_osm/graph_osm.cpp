...
int main(int argc, char **argv) {
  ...
  Bridges *bridges =  new Bridges(16, "esaule", "1090423871824");
	...
	while (1) {
		...
		graph.addVertex(num_vertices, 0);
								// set its location
		graph.getVertex(num_vertices)->getVisualizer()->setLocation(cartesian_coords[0], cartesian_coords[1]);
//		graph.getVertex(num_vertices)->getVisualizer()->setSize(1.0f);
		graph.getVertex(num_vertices)->getVisualizer()->setColor(Color("green"));
		num_vertices++;
	}
...
//cout << "location: " << loc_x << "," << loc_y << endl;
		graph.getVertex(k)->getVisualizer()->setLocation(loc_x, loc_y);
	}
	for (int k = 0; k < num_vertices; k++) {
